...
import io.gridgo.bean.serialization.xml.exceptions.NameAttributeNotFoundException;
import io.gridgo.bean.serialization.xml.exceptions.UnresolvableXmlRefException;
    ...
    private BValue parseValue(Node node, RefManager refManager) {
        ...
        try {
            ...
            case NULL:
                break;
            default:
                if (type.getInnerClass() == null)
                    throw new InvalidTypeException("Cannot parse node with name " + nodeName + " as BValue");
                value.setData(PrimitiveUtils.getValueFrom(type.getInnerClass(), stringValue));
            }
